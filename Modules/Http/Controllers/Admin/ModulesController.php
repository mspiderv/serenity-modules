...
namespace Serenity\Modules\Http\Controllers\Admin;
...
class ModulesController extends AdminController
{
    ...
    public function index()
    {
        ...
        // Box
        $box = GUI::box('Moduly', 'success')->to($this->window);
        ...
        {
            // Column: name
            $name = '<a href="' . route('admin.modules.show', $module->getName()) . '">' . $module->getName() . '</a>';
            ...
            $options = '<a href="' . route('admin.modules.show', $module->getName()) . '" class="table-link">Zobraziť detaily</a>';

            if ( ! $module->isInstalled())
            {
                $options .= '<a href="' . route('admin.modules.install', $module->getName()) . '" class="table-link">Aktivovať modul</a>';

            }
            elseif ( ! $module->isProtected())
            {
                $options .= '<a href="' . route('admin.modules.uninstall', $module->getName()) . '" class="table-link">Deaktivovať modul</a>';
            }

            // Remove link
            if ( ! $module->isProtected())
            {
                $options .= '<a href="' . route('admin.modules.remove', $module->getName()) . '" class="table-link">Odstrániť modul</a>';
            }
    ...
    public function show($moduleName)
    {
        ...
        GUI::button('Späť na zoznam', 'primary')
            ->attr('href', route('admin.modules.index'))
            ->to($buttonGroup);

        if ( ! $module->isInstalled())
        {
            GUI::button('Aktivovať modul', 'success')
                ->attr('href', route('admin.modules.install', $moduleName))
                ->to($buttonGroup);
        }
        elseif ( ! $module->isProtected())
        {
            GUI::button('Deaktivovať modul', 'warning')
                ->attr('href', route('admin.modules.uninstall', $moduleName))
                ->to($buttonGroup);
        }
        ...
        if ( ! $module->isProtected())
        {
            GUI::button('Odstrániť modul', 'danger')
                ->attr('href', route('admin.modules.remove', $moduleName))
                ->to($buttonGroup);
        }
